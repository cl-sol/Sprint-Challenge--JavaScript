...
zooAnimals.map((nameLower) => {
  lowCaseAnimalNames.push(nameLower.animal_name.toLowerCase());
})
...
zooAnimals.filter((popLess5) => {
  if (popLess5.population < 5) {
    lowPopulationAnimals.push(popLess5);
  }
})
...
zooAnimals.reduce((accum, popUS) => {
  return accum + popUS.population;
}, 0);
